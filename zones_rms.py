...
mu=0.4
coloumbs_stress_time_step= 11                     # Time step at which stresses are required  	     
const_stress= -2.5                                   # the separating stress used to define +ve and -ve zones
#range_values=np.zeros([100,2])


              # Since fault is at 20 km so I am taking 5 km distance on both sides
#distance_start= 20000                 
distance_start= 18000
distance_end= 20000  
space_interval=50
...
#	if(kk==6):	
		...
		print(kk)
		problem_name= prob_name + str(kk) + roughness + 'RMS'+ RMS 
	...
	#	print(c_stress.shape
		for dim_y in range(range_of_iter):	
			for dim_x in range(total_dim_x):
				...
				if c_stress[start_x+ dim_x, index_y_start + dim_y ]>= const_stress and c_stress[start_x+ dim_x+1, index_y_start + dim_y ] <const_stress :
						print('inside the loop +')				
						if  c_stress[start_x+ dim_x+2, index_y_start + dim_y] <const_stress  and c_stress[start_x+ dim_x+3, index_y_start + dim_y] <const_stress and c_stress[start_x+ dim_x+4, index_y_start+ dim_y]<const_stress and c_stress[start_x+ dim_x+5, index_y_start + dim_y] <const_stress and c_stress[start_x+ dim_x+6, index_y_start + dim_y] <const_stress and c_stress[start_x+ dim_x+7, index_y_start + dim_y] <const_stress and c_stress[start_x+ dim_x+8, index_y_start + dim_y] <const_stress and c_stress[start_x+ dim_x+9, index_y_start + dim_y] <const_stress and c_stress[start_x+ dim_x+10, index_y_start + dim_y] <const_stress:

							negative_zones[dim_y, kk] = negative_zones[dim_y, kk] +1
							
							neg_zone= 0
							for runs in range (100):
								if c_stress [start_x+ dim_x+1+runs , index_y_start + dim_y] < const_stress:
									neg_zone= neg_zone +1
...
np.savetxt('positive_zones_' + roughness + 'RMS' +RMS +'.out', positive_zones , delimiter=',')
np.savetxt('negative_zones_' + roughness + 'RMS'+ RMS +'.out', negative_zones , delimiter=',')
np.savetxt('full_postv_zone_len_' + roughness + 'RMS'+RMS +'.out', full_pos_len_zone_matrix , delimiter=',')
